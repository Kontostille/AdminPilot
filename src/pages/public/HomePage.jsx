...
function HeroSection() {
  return (
    <section style={{
      background: 'linear-gradient(160deg, var(--ap-dark) 0%, #2D5A43 100%)',
      color: '#FFF', padding: 'var(--space-16) 0 var(--space-20)',
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{
        position: 'absolute', top: -80, right: -80, opacity: 0.04, width: 400, height: 400,
      }}>
        <svg viewBox="0 0 56 56" fill="#FFF" width="100%" height="100%">
          <polygon points="28,2 33,23 52,28 33,33 28,54 23,33 4,28 23,23" />
        </svg>
      </div>
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 640 }}>
          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--ap-mint)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 'var(--space-4)', fontWeight: 500 }}>
          ...
          <h1 style={{ fontSize: 'var(--text-4xl)', fontWeight: 700, lineHeight: 1.15, marginBottom: 'var(--space-4)', color: '#FFF' }}>
            Behördenanträge?<br />
            <span style={{ color: 'var(--ap-gold)' }}>Erledigen wir für Sie.</span>
          </h1>
...
function LeistungswegweiserGrid() {
  const main = LEISTUNGEN.filter(l => ['wohngeld','kv-zuschuss','kindergeld','kinderzuschlag','basiselterngeld','bildung-teilhabe'].includes(l.id));
  const extra = LEISTUNGEN.filter(l => ['kindererziehungszeiten','em-rentenzuschlag'].includes(l.id));
  return (
    <section className="section" style={{ background: '#FFF' }}>
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(155px, 1fr))', gap: 'var(--space-3)', maxWidth: 700, margin: '0 auto' }}>
          {main.map(l => (
            <Link key={l.id} to={l.route} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'var(--space-5) var(--space-3)', background: 'var(--color-bg)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', textDecoration: 'none', textAlign: 'center', transition: 'all var(--transition-base)' }}>
              <span style={{ fontSize: 28, marginBottom: 'var(--space-2)' }}>{l.icon}</span>
              <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--ap-dark)' }}>{l.shortName}</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 'var(--space-6)', marginTop: 'var(--space-4)', flexWrap: 'wrap' }}>
          {extra.map(l => (
            <Link key={l.id} to={l.route} style={{ fontSize: 'var(--text-sm)', color: 'var(--ap-sage)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
    ...
    <section className="section" style={{ background: 'var(--color-bg)' }}>
      <div className="container">
        ...
        <div style={{ textAlign: 'center', marginTop: 'var(--space-8)' }}>
          <Button to="/leistungscheck">In 5 Minuten starten →</Button>
        </div>
      </div>
    </section>
  );
...
function StatsCounter() {
  const stats = [
    { value: '2.500+', label: 'Anträge gestellt' },
    { value: '1,2 Mio €', label: 'für Kunden gesichert' },
    { value: '98%', label: 'Kundenzufriedenheit' },
  ];
  return (
    <section style={{ background: 'var(--ap-dark)', padding: 'var(--space-10) 0' }}>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 'var(--space-6)', textAlign: 'center' }}>
          {stats.map((s, i) => (
            <div key={i}>
              <div style={{ fontSize: 'var(--text-3xl)', fontWeight: 700, color: 'var(--ap-gold)', fontFamily: 'var(--font-mono)' }}>{s.value}</div>
              <div style={{ fontSize: 'var(--text-sm)', color: 'var(--ap-mint)', marginTop: 'var(--space-1)' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
...
function TestimonialSection() {
  const testimonials = [
    { name: 'Maria K.', age: 72, type: 'Rentnerin', leistung: 'Wohngeld', quote: 'Endlich hat sich jemand um meinen Wohngeldantrag gekümmert. Mein Enkel hat mir AdminPilot gezeigt – so einfach!' },
    { name: 'Thomas & Lisa S.', age: 34, type: 'Familie', leistung: 'Kinderzuschlag', quote: 'Wir wussten gar nicht, dass uns Kinderzuschlag zusteht. Jetzt bekommen wir jeden Monat 290 € mehr.' },
    { name: 'Gerhard M.', age: 78, type: 'Rentner', leistung: 'KV-Zuschuss', quote: 'Den KV-Zuschuss hätte ich ohne AdminPilot nie beantragt. Jetzt spare ich über 200 € im Monat.' },
  ];
...
function FAQPreview() {
  ...
  const faqs = [
    { q: 'Was kostet der Service?', a: 'Einmalige Servicegebühr von €150–250 pro Antrag. Kein Abo, keine versteckten Kosten. Der Leistungscheck ist kostenlos.' },
